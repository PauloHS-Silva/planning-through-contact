...
class FaceContactMode(AbstractContactMode):
    ...
    def _define_constraints(self) -> None:
        for idx, lam in enumerate(self.variables.lams):
            self.prog_wrapper.add_bounding_box_constraint(
                idx,
                self.config.contact_config.lam_min,
                self.config.contact_config.lam_max,
                lam,
            )
    ...
    def _define_costs(self) -> None:
        ...
        if cost_config.angular_velocity_regularixation is not None:
            for k, (delta_cos_th, delta_sin_th) in enumerate(
                zip(self.variables.cos_th_vels, self.variables.sin_th_vels)
            ):
                cost = self.prog_wrapper.add_quadratic_cost(
